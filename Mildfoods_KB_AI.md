...
|---|---|---|
| 2026-09-25 | v5.14 | **New §10.2.2: IncomeStatement (`backupStatement`) vs `Profitloss_R2` cross-check.** Revenue agrees within ~0.5% both years; 2026 YTD net agrees within ~9.7K THB (both ≈ -1.6M THB loss); 2025 net differs by ~1.35M THB (IS worse) — gap not yet traced, logged as an open question. Also noted 13 future-dated `TotalAcct` rows. |
...
### 10.2.2 Cross-check: IncomeStatement vs Profitloss_R2 `[VERIFIED 2026-09-25]`

Although the two systems are deliberately separate (§10 header), their **net result per year should land close together**, so comparing them is a useful sanity check on both pipelines. Queries:

```sql
-- IncomeStatement (latest daily snapshot, §11.3)
SELECT Y, MAINCAT, SUM([sum]) FROM inventory.dbo.backupStatement
WHERE Trxdate = (SELECT MAX(Trxdate) FROM inventory.dbo.backupStatement) AND Y IN (2025,2026)
GROUP BY Y, MAINCAT
-- Profitloss (§10.2.1)
SELECT yy, MAINCAT, SUM(THBTotalValue) FROM STAGING.pl.Profitloss_R2 WHERE yy IN (2025,2026) GROUP BY yy, MAINCAT
```

Result (snapshot `Trxdate` 2026-09-26; 2026 = YTD):

| THB | 2025 IncomeStatement | 2025 Profitloss_R2 | Δ | 2026 IncomeStatement | 2026 Profitloss_R2 | Δ |
|---|---|---|---|---|---|---|
| Revenue (IS `01/REVENUE` vs PL `01`+`03`) | 111,819,644.13 | 111,241,854.41 | +577,789.72 | 90,533,348.10 | 90,292,019.24 | +241,328.86 |
| Net (IS `20--NET INCOME--` vs PL sum of all MAINCAT) | -15,719,338.58 | -14,372,238.97 | -1,347,099.61 | -1,604,184.75 | -1,594,443.87 | -9,740.88 |

Findings:
- **Revenue agrees within ~0.5%** both years (IS slightly higher). Expected sources of the gap: IS uses `TotalAcct` `DOC DATE` (cash/accounting side, incl. `02 Other Rev` booked only in accounts), PL uses invoice-level data and excludes `CN%` invoices from `03.Revenue(THB)`.
- **2026 YTD net is essentially identical** (Δ ≈ 9.7K THB, 0.6%). **2025 differs by ~1.35M THB** (PL shows a ~1.35M smaller loss) — not yet traced; likely candidates are year-boundary timing (PL `yy` = invoice year vs IS `Doc Date`/`DeliveryDate` year for `TS-LP-TS%`), costs IS books that PL never allocates to an invoice, and `CGS-LP-OH-003` (excluded from `SumByAccCode` entirely). `[OPEN QUESTION]`
- Both systems now show **2026 YTD as a ~1.6M THB loss**. This supersedes the +5.14M "profit" in §10.2.1 (that was a July snapshot; expenses have caught up since).
- Cost-side categories do NOT map 1:1 (IS groups by account `dep`/`type` — COGS/ADMIN/TRANSPORT/…; PL groups by per-invoice cost component — RM/DL/SM/TS/…) — compare at revenue and net level only, not line by line.
- **Overnight swing 2026-09-25 → 09-26** (`backupProfitloss_R2` snapshots): 2026 net went +5,909,157 → -1,594,444. Almost entirely `01 RM Exp` (-7.1M) with revenue ~flat — cost landed for already-booked invoices, ~6.5M of it from `INV2026-09-EX0024`/`EX0025` (frozen durian, customer WL; RM cost ≈ selling price, each invoice ~-0.5–0.7M after SM/FIX/SB). Lesson: a large YTD profit can be inflated by invoices whose revenue is in but cost isn't yet.
- **Power BI `08. PROFIT_LOSS_detail` showed GP -2,942,385 (Trxdate 26/09/2026, Year 2026) vs SQL -1,594,444.** Reconciles to the baht: Power BI counts `EX0024` + `EX0025` **twice** — revenue 96,866,211.74 = 90,292,019.24 + 6,574,192.50 (their revenue); expense -99,808,597.00 = -91,886,463.11 - 7,922,133.90 (their expenses). SQL side is clean (`GetProfitLoss_R2` join to `Docno_Company_InvName` returns 105 rows/invoice = raw; no dup docno in `MF_Invoice`/`ExchByINV`), so the duplication is inside the Power BI model. **Root cause located (2026-09-26)**: table `dbo.DocNoCompany` holds **2 rows** for each of `EX0024`/`EX0025` (goods owners `WL` CompanyID 17 + `TAWA` CompanyID 114) — the only 2026 invoices with >1 row (29 older docnos 2016–2023 have the same pattern). The view `Docno_Company_InvName` (ROW_NUMBER, 1 row per docno) exists precisely to avoid this, and `GetProfitLoss_R2` uses it — so the model must be relating/merging the fact to `DocNoCompany` (or a table built from it) on docno alone. Confirmed per-invoice in Power BI: EX0024/EX0025 THB & USD exactly ×2, while `EX0023` and `D1069` (1 row each) match SQL; SALE KG is NOT doubled (comes from another table/measure). **Confirmed exact query (owner-supplied M code of the fact table)**: `Sql.Database("mfit.dyndns.biz","inventory",[Query="select pf.*,doc.CustType from inventory.dbo.[backupprofitloss_R2] pf left join inventory.dbo.DocNoCompany doc on pf.InvDocno collate THAI_CI_AS = doc.docno collate THAI_CI_AS"])`. Reproduced in SQL: this join yields 2026 net **-2,942,385.26** (= Power BI), one-row-per-docno yields **-1,594,443.86**. Fix: replace `inventory.dbo.DocNoCompany` with `inventory.dbo.Docno_Company_InvName` in that query (same columns, 1 row per docno), then refresh. `[FIX GIVEN — pending owner apply in Power BI Desktop]`. General rule: **never join `DocNoCompany` on docno alone** — an invoice carrying goods of several owners has one row per owner.
- Side note: `TotalAcct` already holds 13 rows with `DOC DATE` in the future (2026-09-28 … 2026-10-02, ~47K THB total) — post-dated documents, small but they leak into the current-year IS figure.
