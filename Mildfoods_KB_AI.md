...
- **Power BI `08. PROFIT_LOSS_detail` showed GP -2,942,385 (Trxdate 26/09/2026, Year 2026) vs SQL -1,594,444.** Reconciles to the baht: Power BI counts `EX0024` + `EX0025` **twice** — revenue 96,866,211.74 = 90,292,019.24 + 6,574,192.50 (their revenue); expense -99,808,597.00 = -91,886,463.11 - 7,922,133.90 (their expenses). SQL side is clean (`GetProfitLoss_R2` join to `Docno_Company_InvName` returns 105 rows/invoice = raw; no dup docno in `MF_Invoice`/`ExchByINV`), so the duplication is inside the Power BI model (relationship/merge/append) `[OPEN QUESTION — needs Power BI Desktop]`.
